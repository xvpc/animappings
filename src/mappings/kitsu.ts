...
const kitsu = async(title: string) => {
  try{
    ...
    const { data: kData } = await axios.get(`https://kitsu.io/api/edge/anime?filter[text]=${title}`)
    ...
    if (error instanceof Error) {
      console.log(chalk.red`[-] Failed to get mappings for ${title} on Kitsu: ${error.name} - ${error.message}`)
    }
