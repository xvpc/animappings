...
const thetvdb = async(title: string, year?: string, format?: string) => {
  if (!format || format === undefined) format == 'TV'

  console.log(chalk.green(`[+] Getting TVDB mappings for ${title} ${year} ${format}`))

  // if the title includes the words "season", "cour" or part remove it
  title = title.replace(/(season|cour|part)/gi, '').trim()
  
  try{
    const { data: tvdbData } = await axios.post('https://tvshowtime-1.algolianet.com/1/indexes/*/queries?x-algolia-agent=Algolia%20for%20vanilla%20JavaScript%20(lite)%203.32.0%3Binstantsearch.js%20(3.5.3)%3BJS%20Helper%20(2.28.0)&x-algolia-application-id=tvshowtime&x-algolia-api-key=c9d5ec1316cec12f093754c69dd879d3',
      {
        requests: [
          {
            indexName: 'TVDB',
